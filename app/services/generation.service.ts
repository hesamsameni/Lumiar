...
export function useGenerationService() {
  ...
  async function getGenerationsByUser(userId: string, onlyShared = false) {
    let query = supabase
      .from('generations')
      .select('id, output_image_url, prompt, model_name, created_at, metadata, likes(id), is_shared')
      .eq('user_id', userId)
  ...
  async function getExploreGenerations(params: {
    ...
    let query = supabase
      .from('generations')
      .select('id, user_id, output_image_url, prompt, model_name, created_at, metadata, likes(id)')
      .eq('is_shared', true)
      .order('created_at', { ascending: false })
